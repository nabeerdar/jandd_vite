...
import './staff-form-personal-info.css';
import { useNavigate, useParams } from 'react-router-dom';

const DetailedFormPersonalInfo = () => {
    ...
    const handleBack = () => {
      ...
      navigate('/admin');  // This will take the user one step back in history
    };

    const handleNext = () => {
      navigate(`/admin-authorization/${userId}`);
    };
    ...
    useEffect(() => {
      const fetchProfessionalKnowledge = async () => {
        try {
          ...
          // const professionalKnowledgeData = response.data.professional_knowledge;
          // setProfessionalKnowledge(professionalKnowledgeData);
        } catch (err) {
                        ...
                        <tr key={index}>
                          ...
                          <td>
                            <textarea
                              rows="1"
                              cols="20"
                              value={row.school || ""} // Ensure default value
                              onChange={(e) =>
                                handleInputEduCompleted(index, "school", e.target.value)
                              }
                              aria-label={`School name for ${row.level}`}
                            />
                          </td>
                          <td>
                            <textarea
                              rows="1"
                              cols="10"
                              value={row.year || ""} // Ensure default value
                              onChange={(e) =>
                                handleInputEduCompleted(index, "year", e.target.value)
                              }
                              aria-label={`Year of graduation for ${row.level}`}
                            />
                          </td>
                          <td>
                            <textarea
                              rows="1"
                              cols="20"
                              value={row.degree || ""} // Ensure default value
                              onChange={(e) =>
                                handleInputEduCompleted(index, "degree", e.target.value)
                              }
                              aria-label={`Degree for ${row.level}`}
                            />
                          </td>
                        </tr>
                      ))}
  ...
  <label className="sig-form-label">
    ...
    />
        ...
        <div className="p-6 text-center">
                ...
                <p className="mb-4">
                Please follow the link below to check authorization form:
                </p>
                {/* <a
                    href="employee"
...
export default DetailedFormPersonalInfo;
